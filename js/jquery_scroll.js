	...
	$.fn.scrollbar = function(params) {
		...
		return this.each(function() {
			...
			function styler_scrollbar(position,orientation){
				...
				switch (position){
					case 'droite':
						$('.englobe').after('<div class="scrollbar"><div class="bouton">&nbsp;</div></div>');
						$('.scrollbar').css({'margin-left':params.marge_scroll_contenu+'px'});
					break;
					case 'gauche':
						$('.englobe').before('<div class="scrollbar"><div class="bouton">&nbsp;</div></div>');
			...
			if(controle_donnee()){
				...
				switch (params.style) {
					case 'basic':
						...
						$('.scrollbar').css({'width':params.largeur_scrollbar+'px',
											...
											'background':'url(scroll/coba.png)'		
						});
				
						//Style du bouton de la scrollBar
						$(".bouton").css({'width':'35px',
										'height':'42px',
										'background':'url(scroll/scrolIco.png)',
										'top':0+'px',
										'cursor':'pointer',
										'margin-left':'-15px'
										});
				...
				if(params.debug){
					$$.after('<div class="debug"></div>');
					$('.debug').css({'position':'fixed','top':'200px','right':'50px', 'border':'2px solid #EF9700','background':'#FFDC9F','padding':'0px'})				
					.append('<p class="hauteurcontenu">Hauteur du contenu:'+taille_contenu+'</p>')
					.append('<p class="hauteurenglobe">Hauteur de l\'espace visible:'+params.taille_englobe+'</p>')
					.append('<p class="hauteurscrollbar">Hauteur du srcoll:'+params.taille_scrollbar+'</p>')
					.append('<p class="hauteurbouton">Hauteur du bouton:'+params.taille_bouton+'</p>')
					.append('<p class="topcontenu">Top du contenu:'+$$.css('top')+'</p>')
					.append('<p class="topbouton">Top du bouton:'+$('#bouton').css('top')+'</p>')
					.append('<p class="topmaxbouton">Top max du bouton:'+calcul_contenu_top_max()+'</p>')
					.append('<p class="topmaxcontenu">Top max du contenu:'+calcul_bouton_top_max()+'</p>')
